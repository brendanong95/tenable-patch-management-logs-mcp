"""Window-versus-baseline comparison over the logs themselves.
...
enough works. Every threshold is a named constant here and is echoed back in results,
and every finding carries its evidence and a ``reasoning`` sentence.
"""
...
from .knowledge import TIMESTAMP_NOTE
...
def detect_log_anomalies(
    ...
    *,
    ...
    window = build_scope(sources, files, since=since or DEFAULT_WINDOW, until=until)
    ...
    for log_file, entry in scan(scope, coverage):
        ...
        signature_key = (
            (event.device, "-", f"known:{event.known.id}")
            if event.known is not None
            else (event.device, event.entry.component or "-", event.signature)
        )
        record = signatures[signature_key]
    ...
    for (dev, component, signature), record in signatures.items():
        ...
        if record["baseline"] == 0:
            ...
            reasoning = (
                ...
                f"{iso(record['last'])}, and not once in the preceding {baseline_days} day(s) of logs"
            )
            reasoning += (
